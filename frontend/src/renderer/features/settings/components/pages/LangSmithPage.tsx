...
interface LangSmithPageProps {
    config: any;
    handleChange: (key: string, value: any) => void;
}

export function LangSmithPage({ config, handleChange }: LangSmithPageProps) {
    return (
        <SectionCard title="Tracing & Observability" icon={<Gauge className="w-4 h-4" />} color="text-foreground" gradient="from-muted/20 to-muted/5">
            <ToggleGroup
