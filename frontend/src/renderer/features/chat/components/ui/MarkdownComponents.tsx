...
export const mdComponents: ComponentPropsWithoutRef<typeof ReactMarkdown>['components'] = {
    ...
    p: ({ children }) => (
        <p className="mb-4 last:mb-0 leading-[1.75] text-[14.5px] text-foreground/80 font-[450] tracking-[-0.005em]">{children}</p>
    ),
    ...
    h3: ({ children }) => (
        <h3 className="text-lg font-semibold mb-3 mt-6 first:mt-0 text-foreground/90">{children}</h3>
    ),
    h4: ({ children }) => (
        <h4 className="text-base font-semibold mb-2 mt-5 first:mt-0 text-foreground/85">{children}</h4>
    ),
    ...
    // Horizontal rule
    hr: () => (
        <hr className="my-8 border-0 h-px bg-gradient-to-r from-transparent via-foreground/10 to-transparent" />
    ),
    ...
    em: ({ children }) => (
        <em className="text-foreground/60 font-light not-italic">{children}</em>
    ),
